...
On Feb. 1, 2023, the Luxor mining pool mined the largest Bitcoin block (#774628) ever, approximately 4 MB. Most of the blockspace was used to inscribe a Taproot Wizards NFT with [Ordinals](https://ordinals.com/), a project that implements NFTs on Bitcoin by publishing the image data on-chain. Bitcoin NFTs used the Taproot upgrade to inscribe arbitrary data, which increased the amount of data a transaction could inscribe, to a full block’s worth of data (around 4MB). In short, it became easier to post big blobs of data to the Bitcoin blockchain.
